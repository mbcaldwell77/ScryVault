...
export class EbayPricingService {
  ...
  async getPricingData(
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes("rate limit")) {
        ...
        if (expiredCached) {
          console.log(
            `[EbayPricing] Returning expired cache data for ${isbn} due to rate limit`,
          );
  ...
  private async fetchFromEbay(isbn: string): Promise<PricingData | null> {
    ...
    try {
      ...
      if (this.config.userToken) {
        ...
        console.log(
          "[EbayPricing] Using authenticated user token for enhanced data access",
        );
      } else {
        console.log(
          "[EbayPricing] Using App ID only - consider adding user token for better data access",
        );
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 500 && errorText.includes("rate limit")) {
          ...
          this.rateLimitBackoffMs = Math.min(
            this.rateLimitBackoffMs * 2 || 5 * 60 * 1000,
            2 * 60 * 60 * 1000,
          ); // 5min to 2hr
          throw new Error(
            `eBay API rate limit exceeded - backing off for ${this.rateLimitBackoffMs / (60 * 1000)} minutes`,
          );
        }
      ...
      if (searchResult?.item) {
        console.log(
          "[EbayPricing] Sample items:",
          searchResult.item.slice(0, 3).map((item) => ({
            title: item.title?.[0],
            price:
  ...
  private analyzePricingData(
    ...
    Array.from(conditionMap.entries()).forEach(
      ([condition, conditionPrices]) => {
        if (conditionPrices.length > 0) {
          conditionPricing[condition] = {
            averagePrice:
              conditionPrices.reduce((a: number, b: number) => a + b, 0) /
              conditionPrices.length,
    ...
    if (recentSales.length > 0 && olderSales.length > 0) {
      ...
      const olderAvg =
        olderSales.reduce((sum, s) => sum + s.price, 0) / olderSales.length;
      weeklyChange =
        Math.round(((recentAvg - olderAvg) / olderAvg) * 100 * 100) / 100;
  ...
  private normalizeCondition(condition: string): string {
    ...
    if (normalized.includes("new") || normalized.includes("brand new"))
      return "New";
  ...
  private calculateConfidence(
    salesCount: number,
    sales: Array<{ price: number; condition: string; date: string }>,
    ...
    const recent = sales.filter(
      (s) => new Date(s.date).getTime() > Date.now() - 14 * 24 * 60 * 60 * 1000,
    ).length;
  ...
  private async setCachedData(
    ...
    console.log(
      `[EbayPricing] Cached data for ${isbn} with ${cacheDuration / (60 * 60 * 1000)}h memory expiration`,
    );
  }

  private async getDatabaseCachedData(
    isbn: string,
    userId?: number,
  ): Promise<PricingData | null> {
    try {
  ...
  private async setDatabaseCachedData(
    isbn: string,
    data: PricingData,
    userId?: number,
  ): Promise<void> {
  ...
  private async getExpiredCachedData(
    isbn: string,
  ): Promise<PricingData | null> {
    try {
  ...
  private shouldSkipAPICall(): boolean {
    ...
    if (this.lastRateLimitError > 0) {
      ...
      if (timeSinceError > 2 * 60 * 60 * 1000) {
        // 2 hours
